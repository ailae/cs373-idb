...
def artists_and_songs(session):
    ...
    try:
        if not session.query(Artist).first():
            for a in artists:
                artist = Artist(name=a['name'], num_followers=a['num_followers'],
                                artist_id=a['artist_id'], image_url=a[
                                    'image_url'],
        ...
        for s in songs:
            test_song = session.query(Song).filter_by(
                song_id=s['song_id']).first()
            if not test_song:
                song = Song(song_id=s['song_id'], song_name=s['song_name'],
                            artist_id=s['artist_id'], artist_name=s[
                                'artist_name'],
        ...
        for ag in artist_genres:
            artist = session.query(Artist).filter_by(
                artist_id=ag['artist_id']).first()
            if not artist.genres:
                for g in ag['genres']:
                    artist.genres.append(session.query(
                        Genre).filter_by(name=g).first())
                    session.commit()

        # Year top genre relation
        for y in session.query(Year).all():
            if not y.top_genre:
                y.top_genre = session.query(Genre).filter_by(
                    name=y.top_genre_name).first()
                session.commit()
                ...
                if not genre.related_genres:
                    if g['related']:
                        for relations in g['related']:
                            result = session.query(Genre).filter_by(
                                name=relations).first()
                            if result:
        ...
        for ys in year_song:
            # Get the year
            year = session.query(Year).filter_by(
                year=str(ys['year']).strip()).first()
                ...
                for s in ys['song_list']:
                    if s['song_id']:
                        ...
                        if song:
                            ...
                            session.add(assoc)
                            session.commit()
                            # song.years_charted.append(assoc) # Do we need
                            # this?
...
def year(year):
    ...
    a = session.query(Artist).filter_by(
        artist_id=y.top_album_artist_id).first()
...
def visualization():
    ...
    character_counts = [{'text': key, 'count': str(
        character_counts[key])} for key in character_counts]
...
def search(term):
    ...
    queryAndArtist = session.query(Artist,
                                   func.ts_headline('english', Artist.name, func.plainto_tsquery(term)).label('h_name')) \
        .filter(and_(Artist.tsvector_col.match(s) for s in terms)).all()

    queryOrArtist = session.query(Artist,
                                  func.ts_headline('english', Artist.name, func.plainto_tsquery(term)).label('h_name')) \
    ...
    queryAndSong = session.query(Song,
                                 ...
                                 func.ts_headline('english', Song.album_name,
                                                  func.plainto_tsquery(term)).label('h_album_name')) \
    ...
    queryAndYear = session.query(Year,
                                 func.ts_headline('english', Year.year,
                                                  func.plainto_tsquery(term)).label('h_year'),
                                 func.ts_headline('english', Year.top_genre_name,
                                                  func.plainto_tsquery(term)).label('h_top_genre_name'),
                                 func.ts_headline('english', Year.top_album_name,
                                                  func.plainto_tsquery(term)).label('h_top_album_name')) \
    ...
    queryAndGenre = session.query(Genre,
                                  func.ts_headline('english', Genre.name,
                                                   func.plainto_tsquery(term)).label('h_name'),
                                  func.ts_headline('english', Genre.description,
                                                   func.plainto_tsquery(term)).label('h_description')) \
...
def run_tests():
    try:
        result = subprocess.check_output(
            "python3 tests.py", stderr=subprocess.STDOUT, shell=True)
        return result
